...
import { getImageDownsampleSize, getMediaSrc, getPptxPerformanceConfig } from '@/utils/pptxPerformanceMode'
import type {
  Slide,
  TableCellStyle,
  TableCell,
...
const imageDecode = (img: HTMLImageElement) => {
...
const downsampleImageSrc = async (src: string, maxDimension: number) => {
  if (!src || src.startsWith('data:image/svg')) return src

  const img = new Image()
  img.decoding = 'async'
  img.src = src
  await imageDecode(img)

  const size = getImageDownsampleSize({
    width: img.naturalWidth || img.width,
    height: img.naturalHeight || img.height,
    maxDimension,
  })
  if (!size.shouldDownsample) return src
...
const downsampleSlideImages = async (slides: Slide[], maxDimension: number, logContext: Record<string, unknown>) => {
  const cache = new Map<string, Promise<string>>()
  let requested = 0
  let changed = 0
  const startedAt = performance.now()

  const downsample = async (src: string) => {
    if (!src) return src
    requested++
    if (!cache.has(src)) cache.set(src, downsampleImageSrc(src, maxDimension).catch(() => src))
    const nextSrc = await cache.get(src)!
    if (nextSrc !== src) changed++
    return nextSrc
  }
  ...
  for (const slide of slides) {
    if (slide.background?.type === 'image' && slide.background.image?.src) {
      slide.background = {
        ...
        image: {
          ...
          src: await downsample(slide.background.image.src),
        },
      }
    }
    ...
    for (const element of slide.elements) {
      if (element.type === 'image') {
        element.src = await downsample(element.src)
      }
      else if (element.type === 'shape' && element.pattern) {
        element.pattern = await downsample(element.pattern)
      }
    }
  }

  pptistImportLog('import:downsample-images:end', {
    ...logContext,
    requested,
    unique: cache.size,
    changed,
    downsampleMs: elapsedSince(startedAt),
  })
}
...
export default () => {
  ...
  // 导入PPTX文件
  const importPPTXFile = (files: FileList | File[], options?: { cover?: boolean; fixedViewport?: boolean; skipHistorySnapshot?: boolean; logContext?: Record<string, unknown> }): Promise<void> => {
    const defaultOptions = {
      cover: false,
      fixedViewport: false,
    }
    const { cover, fixedViewport, skipHistorySnapshot } = { ...defaultOptions, ...options }
    ...
    const performanceConfig = getPptxPerformanceConfig({
      fileSize: file.size,
      search: window.location.search,
      embedMode: mainStore.embedMode,
    })
    mainStore.setPptxPerformanceMode(performanceConfig.enabled)
    if (performanceConfig.maxCanvasPercentage !== null && mainStore.canvasPercentage > performanceConfig.maxCanvasPercentage) {
      mainStore.setCanvasPercentage(performanceConfig.maxCanvasPercentage)
      mainStore.setCanvasDragged(false)
    }
    pptistImportLog('import:file:start', {
      ...logContext,
      fileName: file.name,
      byteLength: file.size,
      cover,
      fixedViewport,
      skipHistorySnapshot: Boolean(skipHistorySnapshot),
      performanceMode: performanceConfig.enabled,
      performanceReason: performanceConfig.reason,
      parseOptions: performanceConfig.parseOptions,
    })
    ...
    reader.onload = async e => {
      ...
      pptistImportLog('import:file-reader:end', {
        ...logContext,
        fileName: file.name,
      ...
      pptistImportLog('import:parse:start', {
        ...logContext,
        fileName: file.name,
        byteLength: buffer.byteLength,
      })
      try {
        json = await parse(buffer, performanceConfig.parseOptions)
        pptistImportLog('import:parse:end', {
          ...logContext,
          fileName: file.name,
          slides: json.slides?.length ?? 0,
          parseMs: elapsedSince(parseStartedAt),
          totalMs: elapsedSince(importStartedAt),
        })
      }
      catch (err) {
        pptistImportLog('import:parse:error', {
          ...logContext,
          fileName: file.name,
          ...
          for (const el of sortedElements) {
            ...
            if (el.type === 'text') {
              ...
              if (el.shapType === 'line' || /straightConnector/.test(el.shapType) || /bentConnector/.test(el.shapType) || /curvedConnector/.test(el.shapType)) {
                ...
                const pattern: string | undefined = el.fill?.type === 'image' ? getMediaSrc(el.fill.value) : undefined
      ...
      pptistImportLog('import:build-slides:end', {
        ...logContext,
        fileName: file.name,
        slides: slides.length,
        totalMs: elapsedSince(importStartedAt),
      })
      if (performanceConfig.maxImageDimension !== null) {
        await downsampleSlideImages(slides, performanceConfig.maxImageDimension, {
          ...logContext,
          fileName: file.name,
        })
      }
      ...
      if (cover) {
        ...
        if (aspectRatio !== viewportRatio.value) slidesStore.setViewportRatio(aspectRatio)
        if (!skipHistorySnapshot) addHistorySnapshot()
      }
      else if (isEmptySlide.value) {
        slidesStore.setSlides(slides)
        if (aspectRatio !== viewportRatio.value) slidesStore.setViewportRatio(aspectRatio)
      ...
      exporting.value = false
      pptistImportLog('import:store-slides:end', {
        ...logContext,
        fileName: file.name,
        slides: slides.length,
        totalMs: elapsedSince(importStartedAt),
    ...
    reader.onerror = () => {
      pptistImportLog('import:file-reader:error', {
        fileName: file.name,
        message: reader.error?.message || 'FileReader failed',
        totalMs: elapsedSince(importStartedAt),
      })
      exporting.value = false
      resolve()
