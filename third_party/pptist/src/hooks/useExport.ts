...
export default () => {
  ...
  const exportPPTX = (
    ...
    setTimeout(() => {
      // When a host (e.g. the officedex embed) provides a callback, hand back the
      // raw bytes instead of triggering a webview download — desktop webviews can't
      // surface blob downloads, so the host saves the file natively.
      if (onArrayBuffer) {
        pptx.write({ outputType: 'arraybuffer' }).then(buffer => {
          exporting.value = false
          onArrayBuffer(buffer as ArrayBuffer)
        }).catch((err) => {
          exporting.value = false
          onError?.(err)
          message.error('导出失败')
