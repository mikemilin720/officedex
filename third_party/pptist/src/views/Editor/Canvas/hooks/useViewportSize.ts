...
export default (canvasRef: ShallowRef<HTMLElement | null>) => {
  ...
  const { canvasPercentage, canvasDragged, embedMode, embedEditable } = storeToRefs(mainStore)
  const { viewportRatio, viewportSize } = storeToRefs(useSlidesStore())
  const embedEditableTopOffset = 72
  const isEmbedEditableMode = () => embedMode.value && embedEditable.value
  ...
  const initViewportPosition = () => {
    ...
    if (canvasHeight / canvasWidth > viewportRatio.value) {
      ...
      viewportTop.value = isEmbedEditableMode()
        ? embedEditableTopOffset
        : (canvasHeight - viewportActualWidth * viewportRatio.value) / 2
    }
    else {
      ...
      viewportTop.value = isEmbedEditableMode()
        ? embedEditableTopOffset
        : (canvasHeight - viewportActualHeight) / 2
    }
  }
