...
import { normalizeEmbedLocale, translateEmbedUiText } from '@/embedLocale'
...
const remarkLocale = normalizeEmbedLocale(new URLSearchParams(window.location.search).get('lang'))
const speakerNotesPlaceholder = translateEmbedUiText(remarkLocale, '点击输入演讲者备注')
...
onMounted(() => {
  editorView = initProsemirrorEditor((editorViewRef.value as Element), props.value, {
    ...
    placeholder: speakerNotesPlaceholder,
  })
