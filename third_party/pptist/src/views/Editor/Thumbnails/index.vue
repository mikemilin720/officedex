<template>
  <div 
    class="thumbnails"
    :class="{ 'is-embed-mode': embedMode }"
    @mousedown="() => setThumbnailsFocus(true)"
    v-click-outside="() => setThumbnailsFocus(false)"
  >
    <div v-if="!embedMode" class="add-slide">
    ...
    <div
      v-if="embedMode"
      class="thumbnail-list embed-thumbnail-list"
      ref="thumbnailsRef"
    >
      <div
      ...
      >
        ...
        >
          ...
          <ThumbnailSlide class="thumbnail" :slide="element" :size="embedThumbnailSize" :visible="isThumbnailVisible(index)" />
        </div>
      </div>
    </div>
    ...
    <Draggable
      ...
      :disabled="editingSectionId || embedMode"
      @end="handleDragEnd"
      itemKey="id"
      v-contextmenu="embedMode ? undefined : contextmenusThumbnails"
    >
      <template #item="{ element, index }">
        <div class="thumbnail-container">
          ...
          <div
            ...
            @mousedown="$event => handleClickSlideThumbnail($event, index)"
            @dblclick="embedMode ? undefined : enterScreening()"
            v-contextmenu="embedMode ? undefined : contextmenusThumbnailItem"
          >
            <div class="label" :class="{ 'offset-left': index >= 99 }">{{ fillDigit(index + 1, 2) }}</div>
            <ThumbnailSlide class="thumbnail" :slide="element" :size="embedMode ? embedThumbnailSize : 120" :visible="isThumbnailVisible(index)" />
    ...
    <div class="page-number">{{ embedMode ? 'Slide' : '幻灯片' }} {{slideIndex + 1}} / {{slides.length}}</div>
  </div>
</template>
...
const { selectedSlidesIndex: _selectedSlidesIndex, thumbnailsFocus, embedMode, pptxPerformanceMode } = storeToRefs(mainStore)
const { slides, slideIndex, currentSlide } = storeToRefs(slidesStore)
const { ctrlKeyState, shiftKeyState } = storeToRefs(keyboardStore)

const initialThumbnailLoadConfig = getPptxPerformanceConfig({
  embedMode: embedMode.value,
}).thumbnailLoad
const { slidesLoadLimit } = useLoadSlides({
  initialLimit: initialThumbnailLoadConfig.initialLimit,
  batchSize: initialThumbnailLoadConfig.batchSize,
  delayMs: initialThumbnailLoadConfig.delayMs,
  resetOnSlidesChange: initialThumbnailLoadConfig.resetOnSlidesChange,
})

const selectedSlidesIndex = computed(() => [..._selectedSlidesIndex.value, slideIndex.value])
const embedThumbnailSize = 86
const thumbnailLoadConfig = computed(() => getPptxPerformanceConfig({
  embedMode: embedMode.value || pptxPerformanceMode.value,
}).thumbnailLoad)
const isThumbnailVisible = (index: number) => {
  const loadConfig = thumbnailLoadConfig.value
  const limitedVisible = index < Math.min(slidesLoadLimit.value, loadConfig.maxVisible)
  const nearCurrentSlide = Math.abs(index - slideIndex.value) <= loadConfig.neighborRange

  if (!embedMode.value && !pptxPerformanceMode.value) return index < slidesLoadLimit.value
  return nearCurrentSlide || limitedVisible
}

const presetLayoutPopoverVisible = ref(false)

let pendingEmbedSlideSwitchFrame: number | null = null
let pendingEmbedSlideSwitchTimer: ReturnType<typeof window.setTimeout> | null = null
...
// 页面被切换时
const thumbnailsRef = useTemplateRef<InstanceType<typeof Draggable> | HTMLElement>('thumbnailsRef')
const getThumbnailsElement = () => {
  const ref = thumbnailsRef.value
  if (!ref) return null
  return ref instanceof HTMLElement ? ref : ref.$el
}

const clearPendingEmbedSlideSwitch = () => {
  if (pendingEmbedSlideSwitchFrame !== null) {
    window.cancelAnimationFrame(pendingEmbedSlideSwitchFrame)
    pendingEmbedSlideSwitchFrame = null
  }
  if (pendingEmbedSlideSwitchTimer !== null) {
    window.clearTimeout(pendingEmbedSlideSwitchTimer)
    pendingEmbedSlideSwitchTimer = null
  }
}

const postEmbedSlideEvent = (type: 'pptist:slide-changing' | 'pptist:slide-changed', index: number) => {
...
watch(() => slideIndex.value, () => {
  ...
  nextTick(() => {
    const thumbnailsEl = getThumbnailsElement()
    const activeThumbnailRef: HTMLElement | null = thumbnailsEl?.querySelector('.thumbnail-item.active') || null
    if (thumbnailsEl && activeThumbnailRef && !isElementInViewport(activeThumbnailRef, thumbnailsEl)) {
      setTimeout(() => {
...
const changeSlideIndex = (index: number) => {
  ...
  postEmbedSlideEvent('pptist:slide-changing', index)
  clearPendingEmbedSlideSwitch()
  pendingEmbedSlideSwitchFrame = window.requestAnimationFrame(() => {
    pendingEmbedSlideSwitchFrame = null
    pendingEmbedSlideSwitchTimer = window.setTimeout(() => {
      pendingEmbedSlideSwitchTimer = null
      if (slideIndex.value === index) return
      slidesStore.updateSlideIndex(index)
      postEmbedSlideEvent('pptist:slide-changed', index)
    }, 0)
  })
}
...
const handleClickSlideThumbnail = (e: MouseEvent, index: number) => {
  if (embedMode.value) e.preventDefault()
